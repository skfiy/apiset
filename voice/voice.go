...
func TextMp3(content string) string {
	token := fetch_token()
	...
	name := fmt.Sprintf(t+".%v", format_map[AUE])
    return	WriteWithIoutil(name, response)
}
// 获取百度TOKEN
...
func WriteWithIoutil(name, content string) string {
	data :=  []byte(content)
